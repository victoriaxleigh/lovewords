...
| `supabase/migrations/20260731000100_email_invites.sql` | Email / code invites for people not yet on the app (creates `email_invites` + `create_email_invite` / `redeem_email_invite` RPCs; redemption issues a `game_creation_grant` so the game is created through the existing `create_active_game` contract). Required for the invite-by-email feature; the optional `send-invite` function only *delivers* codes it mints | apply once |
